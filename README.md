...
Since LinkedIn doesn't support automated token renewal (unlike Facebook — see below), a scheduled workflow (`.github/workflows/linkedin-token-alert.yml`) runs every 2 weeks (1st and 15th) and checks the `LINKEDIN_TOKEN_EXPIRES_AT` repo **variable** (Settings → Secrets and variables → Actions → **Variables** tab, not Secrets — it's just a date). It automatically opens a GitHub issue titled "LinkedIn token expiring soon" once 14 days or fewer remain — the wider 14-day threshold (vs. the biweekly check interval) guarantees the alert can't be missed between runs. The issue includes the renewal steps above and auto-closes itself once you update the variable to a date more than 14 days out.
